...
- `protocol` 和 `cost_cn`：ModelLink 中国区扩展字段，分别表示调用协议和人民币官方价格。
...
现阶段的验收目标是保持 models.dev 核心字段兼容，并以字段超集提供中国区信息。`protocol` 不依赖 `npm` 判断；`cost_cn` 单位固定为人民币元/百万 Token，并可通过 `cost_cn.thinking` 表达思考模式采用的不同输入、输出单价。国内数据可以没有美元 `cost`。模型同时支持思考和非思考模式时，限制字段按照对应 Provider 的默认模式录入。

同一服务同时兼容 OpenAI 与 Anthropic 协议时，ModelLink 默认使用 OpenAI-compatible 接入地址和 `protocol = "openai-compatible"`；订阅计划与普通按量 API 使用不同地址、模型名称或计费方式时，必须拆成独立 Provider。
